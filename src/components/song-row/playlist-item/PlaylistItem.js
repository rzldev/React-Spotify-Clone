...
function PlaylistItem(props) {
    const { item, index, hoverId, dateOptions, setHoverId, isPlaying, currentlyPlayingSongId, onPlaySong } = props;
    ...
    return (
        <div
            ...
            onMouseOver={() => !showOptions && setHoverId(item.track.id)}
            onMouseLeave={() => !showOptions && setHoverId(null)}
        >
            <span className="w-8 cursor-pointer" onClick={() => onPlaySong(item.track, index)}>
                {
                    (isPlaying && currentlyPlayingSongId === item.track.id)
                        ? <PauseIcon style={{ fontSize: 16 }} />
                        : (hoverId === item.track.id)
            ...
            <div className="w-full grid grid-cols-4 gap-4 items-center">
                <div className="flex gap-2">
                    <img
                        ...
                        className="w-10 h-10" />
                    <div className="grid">
                        <label className="text-md text-white font-medium truncate">{item.track.name}</label>
                                    ...
                                    return (
                                        <Fragment key={index}>
                                            ...
                                            <Link
                                                ...
                                                to={`/${item.track.artists[0].uri.split(':')[1]}/${item.track.artists[0].uri.split(':')[2]}`}
                                            >
                                                {artist.name}
                                            </Link>
                                        </Fragment>
                ...
                <p
                    className="truncate cursor-pointer transition duration-200 ease hover:text-white hover:underline"
                    onClick={() => history.push(`/${item.track.album.uri.split(':')[1]}/${item.track.album.uri.split(':')[2]}`)}
                >
                    {item.track.album.name}
                </p>
                <p>{new Date(item.added_at).toLocaleString("en-US", dateOptions)}</p>
                <div className="relative flex items-center justify-center">
                    {
                        (hoverId === item.track.id) && (
                            <span className={`absolute top-0 left-4 cursor-pointer ${isTrackSaved(libraryCtx.savedTracks, item.track.id) && 'text-spotify'}`} onClick={() => likeSongHandler(item.track)}>
                                {isTrackSaved(libraryCtx.savedTracks, item.track.id) ? <FavoriteIcon style={{ fontSize: 16 }} /> : <FavoriteBorderIcon style={{ fontSize: 16 }} />}
                            </span>
                        )
                    }
                    <p>{`${new Date(item.track.duration_ms).getMinutes()}:${(new Date(item.track.duration_ms).getSeconds() < 10) ? '0' : ''}${new Date(item.track.duration_ms).getSeconds()}`}</p>
                    {
                        (hoverId === item.track.id) && (
